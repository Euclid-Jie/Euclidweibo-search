...
class Get_single_weibo_details:
    ...
    def Get_reposts_info(self):
        ...
        with tqdm(range(0, total_pages + 1)) as t:
            for self.page in t:
                ...
                URL = 'https://weibo.com/ajax/statuses/repostTimeline?id={}&page={}&moduleID=feed&count=10'.format(self.mid, self.pages)
    ...
    def Get_comments_info(self):
        ...
        with tqdm(total=total_comments, desc='{} comments get'.format(self.mid)) as pbar:
            while True:
                ...
                for self.item in comments_list:
                    ...
                    pbar.update(1)
                    pbar.set_postfix({"状态": "已成功写入{}条".format(comments_count)})  # bar's right info
                if max_id == 0:
                    break
                else:
    ...
    def Get_attitudes_info(self):
        ...
        with tqdm(range(0, total_pages + 2)) as t:
            for self.page in t:
                t.set_description("pages:{}".format(self.page))  # bar's left info
                URL = 'https://weibo.com/ajax/statuses/likeShow?id={}&attitude_type=0&attitude_enable=1&page={}&count=20'.format(self.mid, self.pages)
                data_json = Get_json_data(URL, self.header)
                reposts_list = data_json['data']
                for self.item in reposts_list:
